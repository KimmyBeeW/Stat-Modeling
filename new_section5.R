...
for (i in 1:length(ns)) powers[i] <- power.anova.test(groups=5, between.var=var(c(50,50,50,50,60)), 
                           within.var=11.67^2, sig.level=0.05, n=ns[i])$power

plot(ns,powers,type="l",ylim=c(0,1),xlim=c(0,max(ns)),xlab="Sample Size",ylab="Power",
     main="Fruitfly experiment power curves\n  under varying group mean structures") 
     # The "\n" inserts a hard return in the title...so it stretches across two lines

# Power analysis...if we expect one of the groups to be 20 units from the other 4 groups
powersc <- rep(NA,length(ns))
for (i in 1:length(ns)) powersc[i] <- power.anova.test(groups=5, between.var=var(c(0,0,0,0,20)), 
                            within.var=11.67^2, sig.level=0.05, n=ns[i])$power
lines(ns,powersc,col="brown",lty="dotted")

# Power analysis...if we expect alphas to be -5,0,0,0,5
powersd <- rep(NA,length(ns))
for (i in 1:length(ns))powersd[i] <- power.anova.test(groups=5, between.var=var(c(-5,0,0,0,5)), 
                            within.var=11.67^2, sig.level=0.05, n=ns[i])$power
lines(ns,powersd,col="green")

# Power analysis...if we expect copulation groups to have means 10 days longer than non-cop groups
powerse <- rep(NA,length(ns))
for (i in 1:length(ns))powerse[i] <- power.anova.test(groups=5, between.var=var(c(0,0,0,10,10)), 
                            within.var=11.67^2, sig.level=0.05, n=ns[i])$power
...
fly <- read_table("../fruitfly3.txt", 
  ...
  mutate(group=as_factor(group)) %>%   # fly$group <- as.factor(fly$group)
  mutate(group=fct_relevel(group, c("nocomp","preg1","preg8","virgin1","virgin8")))
